...
- **Config via `.env`**: compose interpolates `ADMIN_PASSWORD`, `COOKIE_SECURE`, `MIHOMO_SECRET`, `SUBMERGE_BIND`, and `DOMAIN_RULES_MODE` from a git-ignored `.env` next to `docker-compose.yml` (template: `.env.example`). Built-in defaults keep the localhost case working with no `.env`. `DOMAIN_RULES_MODE` accepts only `report|apply` and defaults to `report`; keep it there until the separate apply installation/migration procedure is complete because the switch alone never makes apply ready. **Internet-facing deploy = copy `.env.example` → `.env`, set a strong `ADMIN_PASSWORD` + `COOKIE_SECURE=true`, and front the loopback-bound `:3000` with a TLS reverse-proxy.**
